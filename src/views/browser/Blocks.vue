...
export default {
  ...
  destroyed () {
	...
	watch: {
		...
		async handler () {
			...
			const res = await axios.get(`http://localhost:8000/api/blocks/?page=${pageNumber}&per_page=${rowsPerPage}`)
  ...
  methods: {
    getLatestBlocks() {
      const self = this;
      //Call to NKN-API https://github.com/CrackDavid/nkn-api
      axios.get('http://localhost:8000/api/blocks/').then(function(response){
